...
class OpenOtherAppsTest {
    ...
    fun test_OpenSettings() {
        ...
        settingsApp.clickAndWaitForNewWindow() // Открываем

        val settingsValidation =
            uiDevice.findObject(UiSelector().packageName("com.android.settings"))
        Assert.assertTrue(settingsValidation.exists())// Убеждаемся, что Настройки открыты
    }
    ...
    /** Хардкод можно заменить на uiDevice.displayWidth/2, uiDevice.displayHeight - 50,
     * uiDevice.displayWidth/2, 0, 10
    Для других устройств список установленных приложений может открываться по другому. Часто это
    иконка на главном экране под названием Apps. Для этого достаточно свернуть все приложения через
    uiDevice.pressHome() и найти Apps на главном экране
    ...
    Apps(будет еще вкладка Widgets). Тогда еще найдем вкладку и выберем ее
    val appsTab: UiObject = uiDevice.findObject(UiSelector().text("Apps")) appsTab.click()
